...
export default function App() {
  ...
  // 전체가 프로젝트 색으로 연하게 펄스하며 그 절정 구간(mobile-detail-burst의
  // 30~50%, 0.7s 중 약 210~350ms) 동안 화면 내용을 바꾸고, 펄스가 걷히는
  // 나머지 구간에는 순수하게 펄스만 사그라들며 이미 바뀐 화면이 드러나게
  // 한다 — 내용이 바뀌는 순간 자체가 펄스와 함께 "보이지" 않아야 자연스럽다
  // (반대 방향도 대칭).
    ...
    return (
      <div className="fixed inset-0 overflow-hidden">
        ...
        {/* 상세 진입·퇴장 색 펄스 — 페이지 전환·호버에 쓰는 gradient-warp-burst와
            같은 언어(부드러운 radial-gradient bloom, 0.7s)를 절정에서 잠깐
            머무는 구간(mobile-detail-burst)만 추가해 재사용한다. 그 구간 동안
            내용이 바뀌므로 전환 자체는 안 보이고, 펄스가 걷히는 나머지
            구간에는 이미 바뀐 화면만 자연스럽게 드러난다. nonce가 바뀔 때마다
            remount돼 애니메이션이 처음부터 재생된다. */}
        {mobilePulse && (
          <div
          ...
          >
            <div
              ...
              style={{
                ...
                marginTop: "-130vw",
                marginLeft: "-130vw",
                background: `radial-gradient(circle, ${hexToRgba(mobilePulse.color, 0.7)} 0%, ${hexToRgba(mobilePulse.color, 0)} 68%)`,
              }}
            />
          </div>
        )}

    ...
    <Page>
      <div className="flex flex-col gap-8">
        <span
          ...
          className="text-xs text-[#0C0F1A]/30 tracking-widest uppercase"
        >
          Fullstack Developer
        </span>
        <h1
  ...
  return (
    <Page>
      <div className="grid md:grid-cols-[180px_1fr] gap-12 items-center">
        <div
          className="bg-[#D8DDE8] overflow-hidden"
        ...
        <div>
          <span
            style={{ fontFamily: "var(--font-mono)" }}
            className="text-xs text-[#0C0F1A]/25 tracking-widest uppercase"
          >
            00 — About
          </span>
          <h2
          ...
          >
            다양한 관점,
            <br />
            <span className="font-semibold">실용적인 코드.</span>
          </h2>
          <div
            style={{ fontFamily: "var(--font-body)" }}
          ...
          >
            <p>
              한양대 ERICA 컴퓨터전공 졸업 (GPA 4.22 / 4.5), 핀테크 인턴십
              풀스택 과정 수료.
            </p>
            <p>
              문제를 발견하면 직접 만들어 해결합니다. 어머니 미용실 앱을 혼자
              설계·개발해 실서비스로, 메신저 통합 문제를 논문으로 발표한 뒤 B2B
              SaaS로 발전시켰습니다.
            </p>
            <p>
              AI를 단순 도구가 아닌 제품의 핵심으로 녹여내는 것을 좋아합니다.
            </p>
          </div>
          <div className="mt-8 grid grid-cols-3 gap-6 border-t border-[#0C0F1A]/8 pt-7">
            {[
              ...
              <div key={l}>
                ...
                <div
                  style={{ fontFamily: "var(--font-mono)" }}
                  className="text-xs text-[#0C0F1A]/30 uppercase tracking-wider mt-1"
                >
                  {l}
                </div>
              </div>
      ...
      <div>
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <h2
              style={{ fontFamily: "var(--font-display)" }}
              className="text-[clamp(1.4rem,3vw,2.2rem)] font-light text-[#0C0F1A] mt-1"
            >
              주요 프로젝트
            </h2>
          </div>
        ...
        <div className="divide-y divide-[#0C0F1A]/8">
          {PROJECTS.map((p) => (
            ...
            >
              <span
                style={{ fontFamily: "var(--font-mono)" }}
                className="text-xs text-[#0C0F1A]/18 w-7 shrink-0"
              >
                {p.id}
              </span>
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline gap-3 flex-wrap">
                  <h3
                    style={{
                      ...
                      fontWeight: hovered === p.id ? 600 : 400,
                    }}
                ...
                <p
                  style={{ fontFamily: "var(--font-body)" }}
                  className="text-xs text-[#0C0F1A]/40 font-light mt-0.5 leading-relaxed"
                >
                  {p.description}
                </p>
              </div>
              <div className="hidden md:flex flex-wrap gap-1.5 w-40 justify-end shrink-0">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    style={{ fontFamily: "var(--font-mono)" }}
                    className="text-xs px-2 py-0.5 bg-[#0C0F1A]/5 text-[#0C0F1A]/40"
                  >
                    {t}
                  </span>
                ))}
              ...
              <span
                style={{ fontFamily: "var(--font-mono)" }}
                className="text-xs text-[#0C0F1A]/20 shrink-0 ml-1"
              >
                {p.year}
              </span>
              <span className="text-[#0C0F1A]/15 group-hover:text-[#4F6EF7] transition-colors duration-200 shrink-0">
                →
              </span>
            </a>
          ))}
  ...
  return (
    <Page>
      <div>
        ...
        <h2
          style={{ fontFamily: "var(--font-display)" }}
          className="text-[clamp(1.4rem,3vw,2.2rem)] font-light text-[#0C0F1A] mt-1 mb-10"
        >
          기술 스택
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
          {SKILLS.map((group) => (
            <div key={group.category}>
              <div
                style={{ fontFamily: "var(--font-mono)" }}
                className="text-xs text-[#4F6EF7] uppercase tracking-widest mb-4"
              >
                {group.category}
              </div>
              <ul className="space-y-2.5">
                {group.items.map((item) => (
                  <li
                    key={item}
                    style={{ fontFamily: "var(--font-body)" }}
                    className="text-sm text-[#0C0F1A]/50 font-light"
                  >
                    {item}
                  </li>
                ))}
        ...
        <div className="mt-12 bg-[#0C0F1A] px-10 py-10">
          ...
          <p
            style={{ fontFamily: "var(--font-mono)" }}
            className="text-xs text-[#F0F3F9]/25 mt-4 uppercase tracking-wider"
          >
            — 오진우
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {EXP_COLS.map((col) => (
            <div key={col.title}>
              <div
                style={{ fontFamily: "var(--font-mono)" }}
                className="text-xs text-[#4F6EF7] uppercase tracking-widest mb-7"
              >
                {col.title}
              </div>
              <div className="space-y-7">
                {col.items.map((item) => (
                  <div key={item.name}>
                    <div
                      style={{ fontFamily: "var(--font-body)" }}
                      className="text-sm font-medium text-[#0C0F1A]"
                    >
                      {item.name}
                    </div>
                    <div
                      style={{ fontFamily: "var(--font-body)" }}
                      className="text-sm text-[#0C0F1A]/40 font-light mt-1 leading-relaxed"
                    >
                      {item.sub}
                    </div>
                    <div
                      style={{ fontFamily: "var(--font-mono)" }}
                      className="text-xs text-[#0C0F1A]/22 mt-1.5"
                    >
                      {item.date}
                    </div>
                  </div>
          ...
          <div className="mt-8 space-y-3.5">
            {[
              ...
              <div key={label} className="flex gap-5 items-baseline">
                <span
                  style={{ fontFamily: "var(--font-mono)" }}
                  className="text-xs text-[#0C0F1A]/25 uppercase tracking-widest w-14 shrink-0"
                >
                  {label}
                </span>
                <span
              ...
              {[
                ...
                <div key={id}>
                  <label
                    htmlFor={id}
                    style={{ fontFamily: "var(--font-mono)" }}
                    className="block text-xs text-[#0C0F1A]/30 uppercase tracking-widest mb-1.5"
                  >
                    {label}
                  </label>
                  <input
